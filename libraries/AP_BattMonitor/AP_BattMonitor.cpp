...
// or removed).
#define ANALOG_RATIOMETRIC

#ifdef ANALOG_RATIOMETRIC
#define BATTERY_VOLTAGE(x) (x->voltage_average_ratiometric()*_volt_multiplier)
#define CURRENT_AMPS(x) ((x->voltage_average_ratiometric()-_curr_amp_offset)*_curr_amp_per_volt)
#else
#define BATTERY_VOLTAGE(x) (x->voltage_average()*_volt_multiplier)
#define CURRENT_AMPS(x) ((x->voltage_average-_curr_amp_offset)*_curr_amp_per_volt)
#endif
...
AP_BattMonitor::read()
{
    ...
    if (_monitoring == AP_BATT_MONITOR_VOLTAGE_ONLY || _monitoring == AP_BATT_MONITOR_VOLTAGE_AND_CURRENT) {
        ...
        _voltage = BATTERY_VOLTAGE(_volt_pin_analog_source);
    }
    ...
    if (_monitoring == AP_BATT_MONITOR_VOLTAGE_AND_CURRENT) {
        ...
        _current_amps = CURRENT_AMPS(_curr_pin_analog_source);
